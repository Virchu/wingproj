...
#fig 5.A
wing.field.line<-
  ggplot(field.sum, aes(x=abs(Latitude), y=mean.wing, group=interaction(Biome,State),colour=State, shape=Biome))+
  geom_errorbar(aes(ymin=mean.wing-se.wing, ymax=mean.wing+se.wing), colour="black",linetype=1,size=.8,width=.6)+
  geom_point(colour="black", size=4,position=position_dodge(0.5), stat="summary", fun.y="mean")+
  geom_point(aes(shape = Biome), size=2.1, position=position_dodge(0.5), stat="summary", fun.y="mean")+
  ggtitle("Average field wing length(mm)")+
  theme(plot.title = element_text(hjust = 0.5), legend.text = element_text(size=10))+
  labs(x="Latitude (S)", y="Wing length (mm)" )+ theme_classic()+
  annotate("text", x = 10, y = 3.4, color = "black", size = 4,label = "y=0.022765x +2.836098  \nR^2=0.2296")+
  geom_abline( intercept=2.84, slope=.023, colour="red", size=.8)

#prep lab wing data
lab.wing.sum <- adult.sum.nosex %>% group_by(Biome, State, mean.Lat)%>%
  summarize (mean.wing=mean(mean.wing, na.rm=TRUE),
             se.wing=mean(se.wing, na.rm=TRUE)) %>% data.frame                 
wing.lab.line<-
  ggplot(lab.wing.sum, aes(x=abs(mean.Lat), y=mean.wing, group=interaction(Biome,State),colour=State, shape=Biome))+
  geom_errorbar(aes(ymin=mean.wing-se.wing, ymax=mean.wing+se.wing), colour="black",linetype=1,size=.8,width=.6)+
  geom_point(colour="black", size=4,position=position_dodge(0.5), stat="summary", fun.y="mean")+
  geom_point(aes(shape = Biome), size=2.1, position=position_dodge(0.5), stat="summary", fun.y="mean")+
  ggtitle("Average lab wing length (mm)")+
  theme(plot.title = element_text(hjust = 0.5), legend.text = element_text(size=10))+
  labs(x="Latitude (S)", y="Wing length (mm)" )+ theme_classic()+
  annotate("text", x = 10, y = 2.9, color = "black", size = 4,label = "y=0.009697x +2.588948  \nR^2=0.1686")+
  geom_abline( intercept=2.59, slope=.0097, colour="red", size=.8)

ggarrange(wing.field.line, wing.lab.line,labels= c("A","B"), ncol=2, nrow=1,  common.legend = TRUE, legend="right")

lmfield<-lm(mean.wing~abs(Latitude), data=field.sum)  
#y=2.79871+0.02801
#R2 = adjusted .8794
lmfield1<-lm(Length.mm~abs(Latitude), data=field_wing)  
#y=2.836098+0.022765
#R2 = adjusted .2296
lmwing<-lm(mean.wing~mean.Lat, data=adult.sum.nosex)
#Wing length
...
cor1<-ggcorr(adult.sum.corsub1[,c(6, 8:10)],
       label=TRUE,
       label_alpha=0.75,
       hjust=0.65,
       layout.exp = 1)
...
## subset correlations by temp
adult.sum.corsub20<- subset(adult.sum.corsub1, adult.sum.corsub1$Temp==1)
adult.sum.corsub24<- subset(adult.sum.corsub1, adult.sum.corsub1$Temp==2)
adult.sum.corsub28<- subset(adult.sum.corsub1, adult.sum.corsub1$Temp==3)

cor2<-ggcorr(adult.sum.corsub20[,c(8:10)],
             label=TRUE,
             label_alpha=0.75,
             hjust=0.65,
             layout.exp = 1)+
  ggplot2::labs(title = "Temperature- 20C")
cor3<-ggcorr(adult.sum.corsub24[,c(8:10)],
             label=TRUE,
             label_alpha=0.75,
             hjust=0.65,
             layout.exp = 1)+
  ggplot2::labs(title = "Temperature- 24C")
cor4<-ggcorr(adult.sum.corsub28[,c(8:10)],
             label=TRUE,
             label_alpha=0.75,
             hjust=0.65,
             layout.exp = 1)+
  ggplot2::labs(title = "Temperature- 28C")

grid.arrange(cor1, cor2, cor3, cor4, ncol=2,nrow=2,top="Correlation between larvae development, wing length and adult lifespan: \n Across all temperatures and within each temperature")
