...
larv_mod1<-lm(sLL~Temp_let*State*Fam_new, data=adar_adult)
anova(larv_mod1)
#posthoc
...
adult_mod2<-lm(AL~Temp_let*Locality, data=adar_adult)
anova(adult_mod2) #both Treatment and Locality sig as well as interaction

adult_mod3<-lmer(AL~Temp_let*Locality+(1|Fam_new), data=adar_adult)
anova(adult_mod3)
AIC(adult_mod, adult_mod1, adult_mod2, adult_mod3)